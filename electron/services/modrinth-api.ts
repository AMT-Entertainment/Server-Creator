...
export class ModrinthAPI {
  searchProjects(query: string, loaders: string[], versions: string[], limit: number = 50): Promise<ProjectResult[]> {
    return new Promise((resolve, reject) => {
      ...
      this.fetchJSON<{ hits: ProjectResult[] }>(url)
        .then(data => {
          resolve(data.hits || []);
        })
        .catch(reject);
  ...
  getProjectVersions(projectId: string): Promise<VersionResult[]> {
    return new Promise((resolve, reject) => {
      ...
      this.fetchJSON<VersionResult[]>(url)
        .then(data => {
          resolve(data || []);
        })
        .catch(reject);
      ...
      protocol
        .get(
          ...
          (response: http.IncomingMessage) => {
            ...
            const _totalSize = parseInt(response.headers['content-length'] || '0', 10);
            let _downloaded = 0;
